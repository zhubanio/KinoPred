...
class DataPreprocessor:
    ...
    def transform(self, df: pd.DataFrame, is_fitting: bool = False) -> pd.DataFrame:
        ...
        if isinstance(df_input, np.ndarray):
            if self.initial_columns is None:
                raise RuntimeError("DataPreprocessor has not been fitted yet. Call fit() before transform().")

            # This part attempts to handle the case where ndarray is passed.
            # The previous error 'numpy.ndarray' object has no attribute 'columns'
            # indicated that df_input was an ndarray at the very beginning of transform.
            # We must convert it to a DataFrame using initial_columns.
            if df_input.shape[1] != len(self.initial_columns):
                # This warning indicates a potential mismatch, but we'll try to proceed.
                print(
                    f"Warning: Input ndarray has {df_input.shape[1]} columns, but expected {len(self.initial_columns)}. "
                    "Column names might be incorrect or missing features. Attempting to proceed with default names.")
                df = pd.DataFrame(df_input, columns=[f'col_{i}' for i in range(df_input.shape[1])])
            else:
                df = pd.DataFrame(df_input, columns=self.initial_columns)
        else:
            df = df_input.copy()  # Keep the copy for safe modification

        # Remove the duplicate df.copy() if it existed here
        # df = df.copy()
        df = df.copy()
